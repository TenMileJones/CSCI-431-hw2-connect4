    ...
    class App(tk.Tk):
        def __init__(self, players=None, player_colors=None, num_columns=7, num_rows=6):
            ...
            if players == None or len(players) == 0:
                self.players = (None, HumanPlayer(), HumanPlayer())
            elif len(players) == 1:
                self.players = (None, HumanPlayer(), players[0])
            elif len(players) == 2:
                self.players = (None, players[0], players[1])
            elif len(players) == 3:
            ...
            if player_colors == None:
                player_color_tuples = (
                None, self._make_color_tuple(DEFAULT_PLAYER1_COLOR), self._make_color_tuple(DEFAULT_PLAYER2_COLOR))
            elif len(player_colors) == 2:
                player_color_tuples = (
                None, self._make_color_tuple(player_colors[0]), self._make_color_tuple(player_colors[1]))

            self.color_strs = (
            None, App._make_color_string(player_color_tuples[1]), App._make_color_string(player_color_tuples[2]))
            self.dark_strs = (None, App._make_color_string(App._darken(player_color_tuples[1])),
                              App._make_color_string(App._darken(player_color_tuples[2])))
            self.light_strs = (None, App._make_color_string(App._lighten(player_color_tuples[1])),
                               App._make_color_string(App._lighten(player_color_tuples[2])))
            ...
            self.canvas = tk.Canvas(width=num_columns * SQUARE_SIZE, height=num_rows * SQUARE_SIZE, bg=BACKGROUND_COLOR,
            ...
            for r in range(num_rows):
                for c in range(num_columns):
                    self.canvas.create_image((c * SQUARE_SIZE + HALF_SQUARE, r * SQUARE_SIZE + HALF_SQUARE),
        ...
        def _declare_victory(self, winner, win_location):
            ...
            coords = [(x[0] * SQUARE_SIZE + HALF_SQUARE, (num_rows - 1 - x[1]) * SQUARE_SIZE + HALF_SQUARE) for x in
                      win_location]
            self.canvas.create_line(coords[0][0], coords[0][1], coords[1][0], coords[1][1],
                                    fill=self.light_strs[winner], width=SQUARE_SIZE / 10, capstyle=tk.ROUND)
        ...
        def _set_player(self, player_id):
            ...
            if type(self.players[player_id]) == HumanPlayer:
                self.top_banner.config(text="Player " + str(self.current_player),
                                       fg=self.color_strs[self.current_player])
            ...
            # if it's an AI, disable buttons & start up its turn
                ...
                self.top_banner.config(text="Player " + str(self.current_player) + " is thinking...",
                                       fg=self.color_strs[self.current_player])
        ...
        def _make_rack_image():
            ...
            im = Image.new("RGBA", (2 * SQUARE_SIZE, 2 * SQUARE_SIZE),
                           App._make_color_tuple(RACK_COLOR))  # (255,255,0,255))
...
def parse_command_line_args(args):
    ...
    # AI file
    if "-f" in args:
        ai_file = args[args.index("-f") + 1].rstrip(".py")
    else:
        ai_file = DEFAULT_AI_FILE

    # number of players
    if "-0" in args:
        players = (ai_file, ai_file)
    elif "-2" in args:
        players = (None, None)
    else:
        players = (None, ai_file)

    # level of players
    if "-l" in args:
        levels = args[args.index("-l") + 1].split(',')
        if len(levels) == 1:
            levels = (int(levels[0]), int(levels[0]))
        else:
            levels = (int(levels[0]), int(levels[1]))
    else:
        levels = (DEFAULT_AI_LEVEL, DEFAULT_AI_LEVEL)
    ...
    if "-n" in args or "--nographics" in args:
        graphics_wanted = False
    else:
        graphics_wanted = True
...
def play_game_in_ascii(player1, player2):
    ...
    while not winning_quartet:
        ...
        if current_player == 1:
            player_escape = P1_ESCAPE
        else:
            player_escape = P2_ESCAPE
        ...
        if type(players[current_player]) == HumanPlayer:
            move = do_human_turn(rack, players[current_player])
        else:
            move = do_computer_turn(rack, players[current_player])
        print()
...
def do_human_turn(rack, player):
    while True:
        ...
        if column >= 0 and column < len(rack) and rack[column][-1] == 0:
            return column
        else:
            print("INVALID")
...
# TODO - this is where rack size is specified
...
def print_rack(rack):
    ...
    for r in range(len(rack[0]) - 1, -1, -1):
        for c in range(len(rack)):
            if rack[c][r] == 1:
                print(P1_ESCAPE + "X" + END_ESCAPE, end=" ")
            elif rack[c][r] == 2:
                print(P2_ESCAPE + "O" + END_ESCAPE, end=" ")
            else:
                print(BOARD_ESCAPE + "." + END_ESCAPE, end=" ")
        print()
